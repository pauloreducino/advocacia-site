...
export default function ArticlesGrid({ articles, categories, initialCategory }: Props) {
  ...
  return (
    <div>
      ...
      {/* Grid */}
      {paged.length === 0 ? (
        <p className="text-muted text-center py-16">Nenhum artigo encontrado.</p>
      ) : (
        <div
        ...
        >
          {paged.map((a) => (
            <article key={a.slug} role="listitem" className="group flex flex-col">
              <Link href={`/blog/${a.slug}`} className="block">
                ...
                <p className="text-[10px] text-gold tracking-[2px] uppercase mb-2">{a.categoryLabel}</p>
                ...
                <p className="text-muted text-[13px] leading-relaxed mb-3 line-clamp-3">{a.excerpt}</p>
