...
export default function CTABanner() {
  return (
    <section
      ...
      style={{
        background: 'linear-gradient(135deg, #0B0F1A 0%, #0F1520 100%)',
      }}
    >
      {/* Decorative law text in background */}
      <div
      ...
      >
        <span
          ...
          style={{ fontSize: 'clamp(6rem, 18vw, 14rem)', opacity: 0.025, letterSpacing: '-0.02em' }}
        >
          Art. 5º
        </span>
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-10 text-center">
        <p className="section-label justify-center reveal mb-6">Próximo passo</p>
        <h2
          className="font-display text-ivory leading-tight mb-5 reveal"
          style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)' }}
        >
          Cada dia sem orientação jurídica
          <br />
          <span className="text-gold">é um risco desnecessário.</span>
        </h2>
        <p className="text-muted text-[16px] mb-10 reveal animate-fade-up-delayed">
          Agende uma consulta confidencial. Sem compromisso.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center reveal animate-fade-up-more">
          <a
            href={getWhatsAppUrl('Olá, gostaria de agendar uma consulta com o Dr. Henrique Cavalcante.')}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Falar com o Dr. Henrique Cavalcante via WhatsApp"
