...
export default async function BlogPreview() {
  ...
  return (
    <section
      id="blog"
      aria-label="Artigos jurídicos recentes"
      className="py-24 max-w-7xl mx-auto px-6 lg:px-10"
    >
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between mb-16 gap-6">
        <div>
          <p className="section-label reveal">Insights jurídicos</p>
        ...
        <Link
          href="/blog"
          className="reveal text-[11px] text-gold border border-gold/40 px-5 py-2.5 rounded-sm hover:bg-gold/10 transition-colors uppercase tracking-widest self-start lg:self-auto"
        >
      ...
      >
        {articles.map((a, i) => (
          <article
            key={a.slug}
            role="listitem"
            className="reveal border-b border-gold/12 pb-8 group"
          ...
          >
            <Link href={`/blog/${a.slug}`} className="block">
              ...
              <h3 className="font-display text-ivory text-[18px] leading-snug mb-3 group-hover:text-gold-light transition-colors duration-200">
                {a.title}
              </h3>
              <p className="text-muted text-[13px] leading-relaxed mb-5 line-clamp-3">
                {a.excerpt}
              </p>
